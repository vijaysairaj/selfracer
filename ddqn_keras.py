...
class DDQNAgent(object):
    ...
    def load_model(self):
        # Load without compiling to avoid deserializing legacy training configs
        # (which may reference deprecated metric names like 'keras.metrics.mse').
        self.brain_eval.model = load_model(self.model_file, compile=False)
        self.brain_eval.model.compile(loss="mse", optimizer="adam")
        self.brain_target.model = load_model(self.model_file, compile=False)
        self.brain_target.model.compile(loss="mse", optimizer="adam")
